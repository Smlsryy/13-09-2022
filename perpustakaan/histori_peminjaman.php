...
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php
        include "conncet.php";
        $qry_histori = mysqli_query($myAdmin, "select * from peminjaman_buku order by id_peminjaman_buku desc");
        ...
        while ($dt_histori = mysqli_fetch_array($qry_histori)) {
            ...
            // tampil buku yg di pinnjam

            $buku_dipinjam = "<ol>";
            $qry_buku = mysqli_query($myAdmin, "select * from detail_peminjaman_buku join buku on buku.id_buku=detail_peminjaman_buku.id_buku where id_peminjaman_buku = '" . $dt_histori['id_peminjaman_buku'] . "'");

            while ($dt_buku = mysqli_fetch_array($qry_buku)) {
                $buku_dipinjam .= "<li>" . $dt_buku['nama_buku'] . "</li>";
            }
            $buku_dipinjam .= "</ol>";
            // tampil status sdh kembali/belum
            $qry_cek_kembali = mysqli_query($myAdmin, "select * from pengembalian_buku where id_peminjaman_buku = '" . $dt_histori['id_peminjaman_buku'] . "'");
